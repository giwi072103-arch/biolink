...
function fillBackgroundTab() {
  ...
  input.addEventListener('change', async () => {
    ...
    try {
      ...
      zone.textContent = 'Нажми, чтобы загрузить фото или видео (до 40МБ)';
    } catch (err) {
...
function fillMusicTab() {
  ...
  input.addEventListener('change', async () => {
    ...
    zone.textContent = 'Загрузить трек с телефона (mp3/wav/m4a, до 20МБ)';
  });
...
function renderTracks() {
  ...
  wrap.innerHTML = '';
  profile.music.tracks.forEach((t) => {
    const row = document.createElement('div');
    row.className = 'track-row';
    row.innerHTML = `<span>${t.title}</span><button class="btn btn--ghost btn--sm">Удалить</button>`;
    row.querySelector('button').addEventListener('click', () => {
